...
int stabilitycheck(struct reb_simulation* r) {
    ...
    for (int i = 0; i < (N - 2); i++) {
        struct reb_particle p0 = r->particles[N - 1];
        struct reb_particle p1 = r->particles[i];
        struct reb_particle p2 = r->particles[i + 1];
        ...
        double dist = pow(p1.x * p1.x + p1.y * p1.y + p1.z * p1.z, 1.0 / 2);

        double Mhill = ((o2.a + o1.a) / 2) * pow(((p1.m + p2.m) / (3 * p0.m)), (1.0 / 3));
        if (dif < (Mhill / 1.0) || dist > maxdist || dist < mindist) {
            return 0;
        }
    }
    
    struct reb_particle plast = r->particles[N - 2];
    double dist = pow(plast.x * plast.x + plast.y * plast.y + plast.z * plast.z, 1.0 / 2);
